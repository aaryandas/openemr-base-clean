...
**PHP program demo data (the Clinical Co-Pilot dataset).** The built-in set is
demographics only. This fork ships a purpose-built synthetic dataset for the
Partial Hospitalization Program use cases in `USERS.md`:
[`sql/php_demo_data.sql`](./sql/php_demo_data.sql), generated by
[`contrib/util/generate_php_demo_data.py`](./contrib/util/generate_php_demo_data.py)
(edit the generator, not the SQL). It registers the five behavioral-health form
modules (PHQ-9, GAD-7, treatment/aftercare plans, transfer summary — their
tables are not in the base schema), then loads 5 staff, 3 therapy groups, and
~21 fictional patients: an 18-patient active census under attending
psychiatrist "Dr. Maya Okafor" (care-team membership is the census source),
one discharged patient with a transfer summary, one patient under a second
attending (out-of-census authorization testing), serial PHQ-9/GAD-7
trajectories, multidisciplinary free-text notes with fresh last-24-hour
activity, medications/problems/allergies, and a **deliberately broken tier**
for eval boundary cases (duplicate-PID pair with the allergy on the twin
chart, zero dates, an incomplete PHQ-9, an empty chart, orphaned rows — see
the generator docstring; do not "fix" these). Dates are relative to `NOW()`,
and the script deletes and re-inserts its own ID ranges (pids 100–129), so it
is safe to re-run any time:

```bash
docker exec -i development-easy-mysql-1 mariadb -uroot -proot openemr \
  < sql/php_demo_data.sql
```

**Even richer data (optional).** Generate
[Synthea](https://github.com/synthetichealth/synthea) FHIR bundles and import
them via OpenEMR's FHIR API (`FHIR_README.md`), or restore a demo database
dump from https://demo.openemr.io.
...
- **Sample data on Railway.** The install path seeds a clean OpenEMR database
  (no demo patients). The live instance has been loaded with the 14 built-in
  demo patients **plus the PHP program dataset** (`sql/php_demo_data.sql`,
  §2.4). To (re)load it, pipe the file into the MySQL service's public proxy
  (get `MYSQL_PUBLIC_URL` via `railway variables --service MySQL`):

  ```bash
  docker run -i --rm -e MYSQL_PWD=<password> mariadb:11.8 \
    mariadb -h <proxy-host> -P <proxy-port> -u root openemr \
    < sql/php_demo_data.sql
  ```
